...
def access_parent(node):
    ...
    """
    group_path = node.store.path
    parent_path = os.path.split(group_path)[0]
    return zarr.hierarchy.group(parent_path)
...
def check_for_attrs_multiscale(ds, multiscale_group, multiscales):
    ...
    if multiscales is not None:
        ...
        scale = os.path.split(ds.store.path)[1]
...
def _read_attrs(ds, order="C"):
    ...
    if not isinstance(ds, zarr.core.Array):
        raise TypeError(
            f"{ds.store.path} is not zarr.core.Array"
        )
